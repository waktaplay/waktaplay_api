...
import StatisticsDate, { type IStatisticsDate } from '../../models/StatisticsDate'
...
async function getDetailData(id: string): Promise<any> {
  try {
    ...
    const data: IMusicData | undefined = thisWeek.find(x => x.id == id)

    const coversData = thisWeek.filter(
      x => String(x.title.simple).split(' - ')[1].includes(String(data?.title.simple).split(' - ')[1]) && x.id != data?.id,
    )
...
router.all('/', async (ctx, next) => {
  try {
    ...
    const hearts: IHearts[] = loadJSON(await Hearts.find({ music: ctx.query.id }))
    ...
    return ctx.body = {
      ...
      data: {
        ...
        isHearted: hearts.filter((x: any) => x.user === userData?.id).length > 0,
      },
      updateDate: updateDate.total,
    }
  } catch (error: any) {
    await errorLog(error, `/api/music/${ctx.query.id}`, process.env.WEBHOOK_URL)
